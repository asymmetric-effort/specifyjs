...
describe("Http502 -- VNode structure", () => {
  ...
  it("passes statusCode 502", () => {
    const vnode = Http502({});
    expect(vnode.props.statusCode).toBe(502);
  });

  it('passes title "Bad Gateway"', () => {
    const vnode = Http502({});
    expect(vnode.props.title).toBe("Bad Gateway");
  });
...
describe("Http502 -- rendered output", () => {
  ...
  it("renders status code 502 in watermark", () => {
    const vnode = Http502({});
    const rendered = vnode.type(vnode.props);
    const children = Array.isArray(rendered.props.children)
      ? rendered.props.children
      : [rendered.props.children];
    expect(children[0].props.children).toBe("502");
  ...
  it("renders title in h1", () => {
    ...
    expect(children[1].props.children).toBe("Bad Gateway");
  });
