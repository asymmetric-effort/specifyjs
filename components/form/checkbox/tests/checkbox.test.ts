...
describe('Checkbox', () => {
  describe('happy paths', () => {
    it('renders with defaults', () => {
      const el = render(createElement(Checkbox, { checked: false, onChange: fn() }));
      const checkbox = el.querySelector('[role="checkbox"]');
      expect(checkbox).toBeTruthy();
    });

    it('renders checked state', () => {
      const el = render(createElement(Checkbox, { checked: true, onChange: fn() }));
      const checkbox = el.querySelector('[role="checkbox"]');
    ...
    it('displays label', () => {
      const el = render(createElement(Checkbox, { checked: false, onChange: fn(), label: 'Accept terms' }));
      expect(el.textContent).toContain('Accept terms');
    });

    it('displays error message', () => {
      const el = render(createElement(Checkbox, { checked: false, onChange: fn(), error: 'Must accept' }));
  ...
  describe('sad paths', () => {
    ...
    it('shows indeterminate state', () => {
      const el = render(createElement(Checkbox, { checked: false, onChange: fn(), indeterminate: true }));
      const checkbox = el.querySelector('[role="checkbox"]');
      expect(checkbox!.getAttribute('aria-checked')).toBe('mixed');
    });

    it('sets aria-disabled when disabled', () => {
      const el = render(createElement(Checkbox, { checked: false, onChange: fn(), disabled: true }));
      const checkbox = el.querySelector('[role="checkbox"]');
