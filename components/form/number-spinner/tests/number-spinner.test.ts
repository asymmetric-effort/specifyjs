...
describe('NumberSpinner', () => {
  describe('happy paths', () => {
    it('renders with defaults', () => {
      const el = render(createElement(NumberSpinner, { value: 0, onChange: fn() }));
      const spinbutton = el.querySelector('[role="spinbutton"]');
      expect(spinbutton).toBeTruthy();
    });

    it('renders controlled value', () => {
      const el = render(createElement(NumberSpinner, { value: 42, onChange: fn() }));
    ...
    it('displays label', () => {
      const el = render(createElement(NumberSpinner, { value: 0, onChange: fn(), label: 'Quantity' }));
      expect(el.textContent).toContain('Quantity');
    });

    it('displays error message', () => {
      const el = render(createElement(NumberSpinner, { value: 0, onChange: fn(), error: 'Min is 1' }));
  ...
  describe('sad paths', () => {
    ...
    it('sets aria-valuenow', () => {
      const el = render(createElement(NumberSpinner, { value: 7, onChange: fn() }));
      const spinbutton = el.querySelector('[role="spinbutton"]');
