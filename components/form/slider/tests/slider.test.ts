...
describe('Slider', () => {
  describe('happy paths', () => {
    it('renders with defaults', () => {
      const el = render(createElement(Slider, { value: 50, onChange: fn() }));
      const slider = el.querySelector('[role="slider"]');
      expect(slider).toBeTruthy();
    });

    it('renders controlled value', () => {
      const el = render(createElement(Slider, { value: 75, onChange: fn() }));
      const slider = el.querySelector('[role="slider"]');
    ...
    it('displays label', () => {
      const el = render(createElement(Slider, { value: 50, onChange: fn(), label: 'Volume' }));
      expect(el.textContent).toContain('Volume');
    });

    it('displays error message', () => {
      const el = render(createElement(Slider, { value: 50, onChange: fn(), error: 'Out of range' }));
  ...
  describe('sad paths', () => {
    it('applies disabled state', () => {
      const el = render(createElement(Slider, { value: 50, onChange: fn(), disabled: true }));
      const slider = el.querySelector('[role="slider"]');
      expect(slider!.getAttribute('tabindex')).toBe('-1');
    });

    it('renders with min value', () => {
      const el = render(createElement(Slider, { value: 0, onChange: fn(), min: 0, max: 100 }));
      const slider = el.querySelector('[role="slider"]');
      expect(slider!.getAttribute('aria-valuenow')).toBe('0');
    });

    it('renders with max value', () => {
      const el = render(createElement(Slider, { value: 100, onChange: fn(), min: 0, max: 100 }));
      const slider = el.querySelector('[role="slider"]');
      expect(slider!.getAttribute('aria-valuenow')).toBe('100');
    });

    it('sets aria-valuemin and aria-valuemax', () => {
      const el = render(createElement(Slider, { value: 50, onChange: fn(), min: 10, max: 90 }));
      const slider = el.querySelector('[role="slider"]');
  ...
  describe('interaction', () => {
    ...
    it('renders range mode with two thumbs', () => {
      const el = render(createElement(Slider, { value: [20, 80], onChange: fn(), range: true }));
